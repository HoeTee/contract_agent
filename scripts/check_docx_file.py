...
def load_clean_docx():
    cleaner_path = PROJECT_ROOT / "tools" / "document" / "file_cleaner.py" # path to the module
    spec = importlib.util.spec_from_file_location("docx_file_cleaner", cleaner_path) # load the module from the specified file path
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Could not load cleaner module from {cleaner_path}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module) # execute the module to populate its namespace
    return module.clean_docx # arguments are passed to this function


clean_docx = load_clean_docx()

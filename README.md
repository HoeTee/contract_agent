...
- `generate_final_report` — 渲染最终审查报告为 DOCX 文件
...
4. **执行与反思**（逐条并发执行）
   - 通过 PageIndex 树搜索合同相关条款
   - 子智能体审查条款是否符合审查标准
   - 反思智能体评估审查结果，不通过则要求修订（循环）
5. **报告汇总** — 将所有审查结果汇编为一份 Markdown 报告
6. **报告生成** — 将最终审查报告和检查清单保存为正式 DOCX 文件
...
- **审查报告**：`docs/reports_md/`（Markdown）及生成的 DOCX 文件路径
...
- **Token 限制**：根据使用的 LLM 调整 `config.py` 中的 `MAX_CONTEXT_TOKENS` 和 `MAX_TOOL_CALLS`
